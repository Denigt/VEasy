...
from tkinter.filedialog import *                                                                                        #|    Programas de terceros:
from winshell import desktop                                                                                            #|      Youtube-dl
...
fich='data.fpy'
try: p_fich=open(fich,'r')
except: r_fich=['funcion: 0', 'tema: 0', 'guardar: '+InitDir]
else:
    r_fich=p_fich.read().split('\n')
    p_fich.close()
#  Excepciones
Except=['"', '*', ':', '<', '>', '?', '|', '\\', '/']
#  Menús
...
def closeWIN(*args):
    if APROCES.get():
        if askyesno('Salir', '¿Aún hay descargas en curso, está seguro de que quiere salir?'):
            WIN.destroy()
    else: WIN.destroy()
#-------------------------------------------------------------------------------------------
...
#--Variables Tkinter------------------------------------------------------------------------
#  Variables del Menú
FUNCION = IntVar(value=int(r_fich[0].split(': ')[1]))
INIT    = IntVar(value=0)
TEMA    = IntVar(value=int(r_fich[1].split(': ')[1]))
...
#--Funciones de Menús-----------------------------------------------------------------------
def Actu_yt():
    threading.Thread(target=f.Update_yt, args=[ESTADO, FUNCION]).start()
    
##def ActuW(*args):
##    estact=['Actualizar Youtube-dl', 'Actualizando Youtube-dl']
##    ESTACT = StringVar(value=estact[UPDATYT.get()])
##    #Ventana
##    ACTU=Toplevel()
##    ACTU.title('Actualizar')
##    ACTU.resizable( width=False, height=False)
##    ACTU.configure(bg=Color[TEMA.get()]['bg'])
##    ACTU.iconbitmap('%s\\VEasy.ico' %(InitDir+Carp['images']))
##    #Widgets
##    FACTU = Frame(ACTU)
##    FACTU.grid(padx=10, pady=10, row=3, column=3, columnspan=3, sticky=S+N+E+W)
##    BACTU = Button(FACTU, textvariable=ESTACT, command= Actu_yt)
##    BACTU.grid(padx=10, pady=10, row=0, rowspan=3, column=0, sticky=S+N+E+W)
##    BACTU.config(bg=Color[TEMA.get()]['bg2'], fg=Color[TEMA.get()]['fg2'], activebackground=Color[TEMA.get()]['abg'], activeforeground=Color[TEMA.get()]['afg'])
##    FACTU.config(bd=Bd[TEMA.get()][1], relief=Borde[TEMA.get()][2], bg=Color[TEMA.get()]['bg'])
...
#  Menú para actualizaciones
MACTU = Menu(MBAR)
MBAR.add_cascade(menu=MACTU, label='Actualizaciones')
MACTU.add_command(label='Actualizar Youtube-dl', command= lambda:Actu_yt())
#  Menú de información
MINFO = Menu(MBAR)
...
def WDownloader(*args):
    ...
    #Descargar playlist
    #CBLIST
    CBLIST = Checkbutton(WIN, text='Descargar Playlist', variable=PLLIST)
    CBLIST.grid(padx=20, row=5, column=5, sticky=S+N+E+W)
    #Elección de Formato
    #LAFORMAT
    ...
    CBLIST.config(bg=Color[TEMA.get()]['bg'], selectcolor=Color[TEMA.get()]['bg3'], fg=Color[TEMA.get()]['fg'], activebackground=Color[TEMA.get()]['bg'], activeforeground=Color[TEMA.get()]['fg'])
...
def searchdir():
    aux=askdirectory(initialdir=SAVE.get())
    if aux!='': SAVE.set(aux.replace('/','\\'))

def searchfile():
    aux=askopenfilename(initialdir=SEARCH.get(), title='Elige un archivo', filetypes=(("All files","*.*"),
...
def Procesar():
    threading.Thread(target=f.Process, args=[WIN, CALIST, FLIST, NAME, URL, SEARCH, SAVE, PLLIST, FORMAT, DELET, NPROCES, APROCES, FUNCION, TEMA]).start()
...
form='funcion: {}\ntema: {}\nguardar: {}'
p_fich=open(fich, 'w')
p_fich.write(form.format(FUNCION.get(), TEMA.get(), SAVE.get()))
p_fich.close()
